...
internal static class Program
{
    ...
    public static string AppDataPath
    {
        get
        {
            if (_appdatapath != "")
                return _appdatapath;
            _appdatapath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\Kayson\";
            return _appdatapath;
        }
    }
        ...
        try
        {
            ...
            if (args.Length > 0)
            {
                if (args[0].ToLower().Trim() == "-reset" && !ei)
                {
                    ...
                    {
                        if (
                            MessageBox.Show("Reset Kayson? This will overwrite all your settings.", "Confirm",
                                            MessageBoxButtons.OKCancel) == DialogResult.OK)
                            EnsureInstall(true);
                    }
                    else
                    {
                        MessageBox.Show("Please exit Kayson before resetting it.");
                    }
                }
