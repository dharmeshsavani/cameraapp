...
namespace iSpyApplication
{
    public partial class NetworkTroubleshooter : Form
    {
        ...
        private void Troubleshooter()
        {
            ...
            UISync.Execute(() => rtbOutput.Text = $"Local Kayson Server: {localserver}{NL}");
            ...
            {
                UISync.Execute(() => rtbOutput.Text += NL+"Warning: Your network adaptor has assigned itself a link-local address (169.254.x.x). This means your PC is setup for DHCP but can't find a DHCP server and Kayson will be unavailable over your LAN. Try resetting your router."+NL);
            }
            ...
            if (r == FireWall.FwErrorCode.FwNoerror)
            {
                if (bOn)
                {
                    string strApplication = Application.StartupPath + "\\Kayson.exe";
                    bool bEnabled = false;
                    fw.IsAppEnabled(strApplication, ref bEnabled);
                    if (!bEnabled)
                    {
                        UISync.Execute(() => rtbOutput.Text += "Kayson is *NOT ENABLED* - add ispy.exe to the windows firewall allowed list");}
                    else
                    {
                        UISync.Execute(() => rtbOutput.Text += "Kayson is enabled");
                    }
                }
                else
                {
            ...
            else
            {
                ...
                if (Convert.ToBoolean(result[1]))
                {
                    ...
                    if (result.Length>3 && result[3] != "")
                    {
                        ...
                        if (!bIPv6)
                        {
                            ...
                            if (portMapOk)
                            {
                                UISync.Execute(
                                    ...
                                    " or check if your antivirus protection (eset, zonealarm etc) is blocking Kayson. ");
                            }
                        }

                        if (MainForm.AddressListIPv4.Length > 1)
                        {
                            UISync.Execute(() => rtbOutput.Text += NL+"Warning: There are multiple network adaptors in your PC. Try selecting a different IP address to listen on in Kayson web settings or disable unused network adaptors and restart Kayson: " + NL);
                        ...
                        UISync.Execute(() => rtbOutput.Text += NL + NL + "Please see the troubleshooting section here: http://www.Kaysonconnect.com/userguide-remote-access-troubleshooting.aspx");
                ...
                {
                    UISync.Execute(() => rtbOutput.Text += NL +
                                      "Not subscribed - local access only. http://www.Kaysonconnect.com/subscribe.aspx");
