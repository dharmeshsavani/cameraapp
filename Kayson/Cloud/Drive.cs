...
namespace iSpyApplication.Cloud
{
    public static class Drive
    {
        ...
        public static string Upload(string filename, string path, out bool success)
        {
            ...
            UploadList.Add(new UploadEntry { DestinationPath = "Kayson\\" + path.Replace("/", "\\").Trim('\\'), SourceFilename = filename });
            if (!_uploading)
            {
                _uploading = true;
        ...
        private static void Upload(object state)
        {
            ...
            if (Service != null)
            {
                ...
                var body = new File {Title = fi.Name, Description = "Kayson", MimeType = mt};
